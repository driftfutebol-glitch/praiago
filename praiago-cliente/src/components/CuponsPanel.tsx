          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                <Ticket size={19} color="var(--pg-warning)" />
                <span style={{ fontSize: 17, fontWeight: 900, color: 'var(--pg-ink)' }}>Cupons</span>
              </div>
              <button type="button" onClick={onFechar} aria-label="Fechar" style={{ border: 0, background: 'var(--pg-surface-alt)', borderRadius: 999, width: 32, height: 32, cursor: 'pointer', display: 'grid', placeItems: 'center' }}>
                <X size={16} color="var(--pg-muted)" />
              </button>
            </div>

            {/* Resgate por código */}
            <div style={{ background: 'var(--pg-input)', border: '1px solid var(--pg-line)', borderRadius: 18, padding: 14, marginBottom: 16 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 9 }}>
                <KeyRound size={15} color="var(--pg-ocean)" />
                <span style={{ fontSize: 13, fontWeight: 900, color: 'var(--pg-ink)' }}>Tenho um código</span>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                <input
                  ...
                  spellCheck={false}
                  style={{
                    flex: 1, minWidth: 0, background: 'var(--pg-surface)', color: 'var(--pg-ink)',
                    border: '1px solid var(--pg-line)', borderRadius: 14,
                ...
                <button
                  ...
                  style={{
                    ...
                    background: (!codigo.trim() || conferindo) ? 'var(--pg-line)' : 'var(--pg-brand-gradient)',
                    color: (!codigo.trim() || conferindo) ? 'var(--pg-faint)' : 'var(--pg-on-brand)',
              ...
              {erro && (
                <div style={{ marginTop: 9, fontSize: 12, fontWeight: 750, color: 'var(--pg-danger)', lineHeight: 1.4 }}>{erro}</div>
              )}

              {resgatado && (
                <div style={{ marginTop: 11, background: 'var(--pg-success-bg)', border: '1px solid var(--pg-success)', borderRadius: 14, padding: '11px 13px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                    <Check size={15} color="var(--pg-success)" />
                    <span style={{ fontSize: 13, fontWeight: 900, color: 'var(--pg-success)' }}>{resgatado.titulo}</span>
                    <span style={{ fontSize: 10.5, fontWeight: 900, color: 'var(--pg-success)', background: 'var(--pg-surface)', padding: '3px 8px', borderRadius: 999 }}>
                      {textoDesconto(resgatado.tipo, resgatado.valor)}
                    </span>
                  </div>
                  <div style={{ marginTop: 5, fontSize: 11.5, fontWeight: 700, color: 'var(--pg-success)', lineHeight: 1.4 }}>
            ...
            {/* Lista pública */}
            <div style={{ fontSize: 12.5, fontWeight: 900, color: 'var(--pg-muted)', textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>
              Disponíveis agora
            </div>

            {cupons.length === 0 ? (
              <div style={{ borderRadius: 18, padding: 16, background: 'var(--pg-warning-bg)', border: '1px dashed var(--pg-warning)', fontSize: 12.5, fontWeight: 700, color: 'var(--pg-warning)', lineHeight: 1.5 }}>
              ...
              <div style={{ display: 'grid', gap: 10 }}>
                {cupons.map(c => (
                  <div key={c.id} style={{ borderRadius: 18, padding: 14, background: 'var(--pg-warning-bg)', border: '1px dashed var(--pg-warning)', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 44, height: 44, flexShrink: 0, borderRadius: 15, background: 'var(--pg-warning-bg)', color: 'var(--pg-warning)', display: 'grid', placeItems: 'center' }}>
                      <Ticket size={21} />
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
                        <span style={{ fontSize: 14, fontWeight: 900, color: 'var(--pg-warning)' }}>{c.titulo}</span>
                        <span style={{ fontSize: 10.5, fontWeight: 900, color: 'var(--pg-warning)', background: 'var(--pg-surface)', padding: '3px 8px', borderRadius: 999 }}>
                          {textoDesconto(c.tipo, c.valor)}
                        </span>
                      </div>
                      <div style={{ fontSize: 11.5, fontWeight: 750, color: 'var(--pg-warning)', marginTop: 4 }}>
                        {c.codigo}
                        {c.valor_minimo > 0 && ` · mínimo R$ ${Number(c.valor_minimo).toFixed(2).replace('.', ',')}`}
                      </div>
                      {c.validade && (
                        <div style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--pg-warning)', marginTop: 2 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: copiado === c.codigo ? 'var(--pg-success-bg)' : 'var(--pg-surface)',
                        color: copiado === c.codigo ? 'var(--pg-success)' : 'var(--pg-warning)',
