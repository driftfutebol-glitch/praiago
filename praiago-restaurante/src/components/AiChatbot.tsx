...
export default function AiChatbot({ plataforma = 'restaurante' }: { plataforma?: string }) {
  const [isOpen, setIsOpen] = useState(false)
  const [messages, setMessages] = useState<Message[]>([
    { id: 'welcome', role: 'bot', text: 'Ola! Como o suporte PraiaGo pode te auxiliar?' }
  ])
...
1. A PraiaGo é uma plataforma de tecnologia que conecta consumidores a estabelecimentos parceiros.
2. Somos responsáveis pela interface e intermediação, mas a execução, qualidade e entrega do produto são de inteira responsabilidade do estabelecimento parceiro.
3. Pagamentos são processados via parceiros financeiros diretamente para o estabelecimento.
4. Questões operacionais como cancelamentos, reembolsos ou problemas no pedido devem ser tratadas com o estabelecimento ou através do nosso suporte oficial.
5. Operamos em estrita conformidade com a LGPD, tratando dados apenas para a finalidade de viabilizar o pedido.
Nunca invente dados. Se o usuário quiser falar com um humano, mande digitar "suporte".` },
  ...
  const startTicketFlow = () => {
    setMode('ticket')
    addMessage('bot', 'Certo. Qual é o assunto do suporte? (Ex: Problema com o repasse, Erro no painel)')
  }
  ...
  const handleSendTicket = async (e?: React.FormEvent) => {
    ...
    if (!ticketSubject) {
      ...
      addMessage('bot', 'Por favor, me dê mais detalhes sobre esse problema.')
      return
    }
    ...
    try {
      ...
      addMessage('bot', '✅ O chamado foi criado com sucesso no painel administrativo. Vamos analisar o mais rápido possível.')
      setMode('ai')
      setTicketSubject('')
    } catch (err) {
      console.error(err)
      addMessage('bot', '❌ Erro de comunicação com a base. Tente novamente.')
    } finally {
      setLoading(false)
    }
  }
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div style={{
              padding: '16px 20px',
              background: 'linear-gradient(90deg, rgba(249,115,22,0.1), rgba(234,88,12,0.1))',
              ...
              {messages.map(msg => (
                <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: msg.role === 'bot' ? 'flex-start' : 'flex-end' }}>
                  <div style={{
                    ...
                    background: msg.role === 'bot' ? 'rgba(0,0,0,0.05)' : '#f97316',
            ...
            {mode === 'ai' && (
              <div style={{ padding: '0 20px 10px', display: 'flex', gap: 8, overflowX: 'auto' }} className="hide-scrollbar">
                <button
                  onClick={() => setInput('Problema com pedido')}
            ...
            <form onSubmit={mode === 'ai' ? handleSendAI : handleSendTicket} style={{
              ...
              <input
                ...
                placeholder={mode === 'ticket' ? "Detalhes..." : "Mensagem..."}
                disabled={loading}
                style={{
              ...
              <button
                ...
                style={{
                  ...
                  background: input.trim() && !loading ? '#f97316' : 'rgba(0,0,0,0.05)',
