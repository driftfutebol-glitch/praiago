...
function prepareManifest(value: unknown): PreparedProduct[] {
  ...
  for (const product of manifest.products) {
    ...
    for (const variant of variants) {
      ...
      prepared.push({ name: fullName, description: image ? `${description}\nFoto ilustrativa.` : description, category, price, image })
    }
  }
...
export default function BulkProductImport({ sellerId, existingNames, onClose, onImported }: {
  sellerId: string
  ...
  const existing = useMemo(() => new Set(existingNames.map(name => name.toLocaleLowerCase('pt-BR'))), [existingNames])
  ...
  async function importProducts() {
    if (saving || pending.length === 0 || missing.length) return
    setSaving(true)
    setError('')
    let completed = 0
    try {
      ...
      setProgress(`${completed} itens importados com sucesso.`)
      await onImported()
    } catch (cause) {
      setError(`Importação interrompida após ${completed} item(ns). ${cause instanceof Error ? cause.message : 'Tente novamente.'} Reabra o arquivo para continuar sem duplicar os já salvos.`)
      await onImported()
    } finally {
      setSaving(false)
    }
  ...
  return (
    <div role="dialog" aria-modal="true" aria-label="Importar cardápio" style={{ position: 'fixed', inset: 0, zIndex: 12000, background: 'rgba(15,23,42,.72)', display: 'grid', placeItems: 'center', padding: 16 }}>
      <div style={{ width: 'min(100%, 560px)', maxHeight: '90vh', overflowY: 'auto', background: '#fff', color: '#0f172a', borderRadius: 22, padding: 24, boxShadow: '0 24px 70px rgba(0,0,0,.28)' }}>
        ...
        <p style={{ color: '#475569', lineHeight: 1.5 }}>Selecione o JSON de produtos e as fotos indicadas nele. Cada tamanho vira um item com seu próprio preço. Itens já cadastrados pelo mesmo nome serão ignorados.</p>
        ...
        {products.length > 0 && <p role="status">{products.length} itens no arquivo · {pending.length} novos · {products.length - pending.length} já cadastrados.</p>}
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          ...
          <button type="button" disabled={saving || !pending.length || missing.length > 0} onClick={() => void importProducts()} style={{ padding: '12px 18px', borderRadius: 12, border: 0, color: '#fff', background: '#ea580c', cursor: 'pointer', opacity: saving || !pending.length || missing.length ? .5 : 1 }}>Importar {pending.length} itens</button>
        </div>
      </div>
    </div>
  )
