// Pedido de troca do nome da loja (precisa de aprovacao do admin).
//
...
// qualquer loja "renascer limpa" depois de uma sequencia de notas ruins — a
...
/** Colapsa espacos repetidos: "Bar  do  Ze" e "Bar do Ze" sao o mesmo nome. */
...
export function validarPedidoTrocaNome(nomeNovo: string, nomeAtual: string, motivo: string): string {
  ...
  if (novo.toLowerCase() === normalizarNomeLoja(nomeAtual).toLowerCase()) {
    return 'Esse ja e o nome atual da loja.'
  }
